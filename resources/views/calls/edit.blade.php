...
@section('title', 'Edit Call Log - ' . brand_name())
...
@section('content')
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-sm border border-[#E5DED4] p-6">
            ...
            <form method="POST" action="{{ route('calls.update', $callLog->id) }}" id="callForm">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <select name="lead_id" id="lead_id" required
                            ...
                            @foreach($leads as $lead)
                                <option value="{{ $lead->id }}" {{ old('lead_id', $callLog->lead_id) == $lead->id ? 'selected' : '' }}
                    ...
                    <div>
                        ...
                        <input type="text" name="phone_number" id="phone_number" required
                               value="{{ old('phone_number', $callLog->phone_number) }}"
                               class="w-full px-4 py-2 border border-[#E5DED4] rounded-lg focus:ring-2 focus:ring-[#205A44] focus:border-[#205A44]">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="start_time" id="start_time" required
                               value="{{ old('start_time', $callLog->start_time->format('Y-m-d\TH:i')) }}"
                               class="w-full px-4 py-2 border border-[#E5DED4] rounded-lg focus:ring-2 focus:ring-[#205A44] focus:border-[#205A44]">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="end_time" id="end_time"
                               value="{{ old('end_time', $callLog->end_time ? $callLog->end_time->format('Y-m-d\TH:i') : '') }}"
                               class="w-full px-4 py-2 border border-[#E5DED4] rounded-lg focus:ring-2 focus:ring-[#205A44] focus:border-[#205A44]">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="duration" id="duration" min="0"
                               value="{{ old('duration', $callLog->duration) }}"
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="missed" {{ old('status', $callLog->status) == 'missed' ? 'selected' : '' }}>Missed</option>
                            <option value="rejected" {{ old('status', $callLog->status) == 'rejected' ? 'selected' : '' }}>Rejected</option>
                            <option value="busy" {{ old('status', $callLog->status) == 'busy' ? 'selected' : '' }}>Busy</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="call_outcome" id="call_outcome"
                            ...
                            <option value="interested" {{ old('call_outcome', $callLog->call_outcome) == 'interested' ? 'selected' : '' }}>Interested</option>
                            <option value="not_interested" {{ old('call_outcome', $callLog->call_outcome) == 'not_interested' ? 'selected' : '' }}>Not Interested</option>
                            <option value="callback" {{ old('call_outcome', $callLog->call_outcome) == 'callback' ? 'selected' : '' }}>Callback Requested</option>
                            <option value="no_answer" {{ old('call_outcome', $callLog->call_outcome) == 'no_answer' ? 'selected' : '' }}>No Answer</option>
                            <option value="busy" {{ old('call_outcome', $callLog->call_outcome) == 'busy' ? 'selected' : '' }}>Busy</option>
                            <option value="other" {{ old('call_outcome', $callLog->call_outcome) == 'other' ? 'selected' : '' }}>Other</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="next_followup_date" id="next_followup_date"
                               value="{{ old('next_followup_date', $callLog->next_followup_date ? $callLog->next_followup_date->format('Y-m-d\TH:i') : '') }}"
                               class="w-full px-4 py-2 border border-[#E5DED4] rounded-lg focus:ring-2 focus:ring-[#205A44] focus:border-[#205A44]">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="4"
                                  ...
                                  class="w-full px-4 py-2 border border-[#E5DED4] rounded-lg focus:ring-2 focus:ring-[#205A44] focus:border-[#205A44]">{{ old('notes', $callLog->notes) }}</textarea>
                    </div>
                </div>
                ...
                <div class="mt-6 flex gap-4">
                    <button type="submit" class="px-6 py-2 btn-brand-gradient text-white rounded-lg transition-colors duration-200 font-medium">
                        <i class="fas fa-save mr-2"></i> Update Call Log
                    </button>
                    <a href="{{ route('calls.show', $callLog->id) }}" class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors duration-200 font-medium">
                        Cancel
                    </a>
                </div>
            </form>
