...
<script>
    ...
    async function changePasswordOnly(event) {
        if (event) {
        ...
        const originalText = changePasswordBtn ? changePasswordBtn.textContent : 'Change Password';

        if (!currentPassword || !newPassword || !confirmPassword) {
            showAlert('passwordAlert', 'All password fields are required', 'error');
            return;
        }
        ...
        try {
            ...
            if (response.ok && result.success !== false) {
                showAlert('passwordAlert', result.message || 'Password changed successfully', 'success');
    ...
    document.getElementById('passwordForm')?.addEventListener('submit', changePasswordOnly);

    window.saveAllChanges = saveAllChanges;
    window.changePasswordOnly = changePasswordOnly;
