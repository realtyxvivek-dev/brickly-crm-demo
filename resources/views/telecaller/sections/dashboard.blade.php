...
<div class="bg-white rounded-lg shadow p-6 mb-6" id="telecallerIncentivesSection">
    ...
    <div class="mb-4">
        <h3 class="text-lg font-semibold text-gray-800 mb-3">Rejected Incentives</h3>
        <div id="telecallerRejectedIncentivesList" class="space-y-2">
            <p class="text-gray-500 text-sm">No rejected incentives</p>
        </div>
    </div>
...
<script>
    ...
    function loadTelecallerIncentivesData(incentives) {
                ...
                <div class="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div class="flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-xs text-gray-600">Status: ${inc.status === 'pending_sales_head' ? 'Awaiting Sales Head' : (inc.status === 'pending_crm' ? 'Awaiting CRM' : 'Awaiting Finance')}</p>
                        </div>
        ...
        const rejectedList = document.getElementById('telecallerRejectedIncentivesList');
        if (incentives.rejected && incentives.rejected.length > 0) {
            rejectedList.innerHTML = incentives.rejected.map(inc => `
                <div class="p-3 bg-rose-50 border border-rose-200 rounded-lg">
                    <div class="flex justify-between items-start gap-3">
                        <div>
                            <p class="text-sm font-medium text-gray-900">${inc.site_visit?.customer_name || 'N/A'}</p>
                            <p class="text-xs text-rose-700 mt-1"><strong>Remark:</strong> ${inc.rejection_reason || 'No remark shared.'}</p>
                        </div>
                        <span class="text-lg font-bold text-rose-700">â‚¹${parseFloat(inc.amount).toFixed(2)}</span>
                    </div>
                    <div class="mt-3">
                        <button onclick="requestSiteVisitIncentive(${inc.site_visit?.id || 'null'}, ${parseFloat(inc.amount || 0)})"
                                class="px-4 py-2 bg-gradient-to-r from-rose-600 to-rose-700 text-white rounded-lg text-sm font-medium hover:from-rose-700 hover:to-rose-800 transition-all">
                            <i class="fas fa-rotate-right mr-1"></i>Resubmit Incentive
                        </button>
                    </div>
                </div>
            `).join('');
        } else {
            rejectedList.innerHTML = '<p class="text-gray-500 text-sm">No rejected incentives</p>';
        }
            ...
            verifiedList.innerHTML = incentives.verified.slice(0, 5).map(inc => `
                <div class="p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div class="flex justify-between items-center">
                        <div>
                            <p class="text-sm font-medium text-gray-900">${inc.site_visit?.customer_name || 'N/A'}</p>
                            <p class="text-xs text-gray-600">Verified on ${(inc.finance_manager_verified_at || inc.crm_verified_at || inc.sales_head_verified_at) ? new Date(inc.finance_manager_verified_at || inc.crm_verified_at || inc.sales_head_verified_at).toLocaleDateString('en-IN') : 'N/A'}</p>
                        </div>
    ...
    function requestSiteVisitIncentive(siteVisitId, amount = '') {
